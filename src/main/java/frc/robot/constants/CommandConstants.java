package frc.robot.constants;

import frc.robot.common.CanPort;

public final class CommandConstants {
    
    public static final int INTAKE_UP_ENCODERVALUE = 0;//placeholder may need to be changed
    public static final int INTAKE_DOWN_ENCODERVALUE = 10000;//placeholder may need to be changed
    public static final int INTAKE_AMP_ENCODERVALUE = 7000;//placeholder may need to be changed

    public static final double INTAKE_PICKUP_SPEED = 0.5;//placeholder
    public static final double INTAKE_SLOW_SPEED = 0.3;
    public static final double INTAKE_FAST_SPEED = 6;
    public static final double INTAKE_STALL_SPEED =0.1;


    public static final double SHOOTER_INTAKE_SPEED = 0.5;
    public static final double SHOOTER_STALL_SPEED = 0.1;//placeholder may need to be changed



    
    
 

}